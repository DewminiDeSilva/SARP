...
<head>
  ...
  <title>Add New Quarter Progress Report</title>
  <link
...
<div class="frame" style="padding-top: 70px;">
    ...
    <h3 class="page-title">Add New Quarter Progress Report</h3>

    <form action="{{ route('progress.quarterly.store') }}" method="POST" enctype="multipart/form-data" class="modern-form">
      @csrf
      <div class="row g-4">
        <div class="col-md-4">
        ...
        <div class="col-md-4">
          ...
          <select name="period_quarter" class="form-select" required>
            ...
            @foreach($quarters as $q)
              <option value="{{ $q }}" @selected(old('period_quarter')==$q)>Q{{ $q }}</option>
            @endforeach
          </select>
          @error('period_quarter') <small class="text-danger">{{ $message }}</small> @enderror
        </div>
