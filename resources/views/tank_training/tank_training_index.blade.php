...
<head>
    ...
    <title>Tank Training Program</title>
        ...
        <div class="d-flex align-items-center mb-3">
            ...
            <div class="container-fluid">
                <div class="center-heading text-center">
                    <h1 style="font-size: 2.5rem; color: green;">Tank Training Program Details</h1>
                </div>
                    ...
                    <div class="row justify-content-center mt-4">
                        ...
                        <div class="card text-center" style="width: 18rem;">
                            ...
                            <div class="card-body">
                                ...
                                <p class="card-text">Total number of tank training programs.</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    @if(auth()->user()->hasPermission('tank_training', 'upload_csv'))
                    <form action="{{ route('tank-training.upload_csv') }}" method="POST" enctype="multipart/form-data" class="form-inline">
                        @csrf
                    ...
                    @endif
                    <form method="GET" action="{{ route('tank-training.index') }}" class="form-inline">
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div class="entries-container">
                        ...
                        <select id="entriesSelect" class="custom-select custom-select-sm form-control form-control-sm mx-2">
                            <option value="10" {{ $tankTrainings->perPage() == 10 ? 'selected' : '' }}>10</option>
                            <option value="25" {{ $tankTrainings->perPage() == 25 ? 'selected' : '' }}>25</option>
                            <option value="50" {{ $tankTrainings->perPage() == 50 ? 'selected' : '' }}>50</option>
                            <option value="100" {{ $tankTrainings->perPage() == 100 ? 'selected' : '' }}>100</option>
                        </select>
                ...
                <div class="row table-container">
                    <div class="col">
                        ...
                        <table id="trainingTable" class="table table-bordered">
                            ...
                            <tbody>
                                @foreach ($tankTrainings as $tt)
                                <tr>
                                    <td class="program-name">{{ $tt->program_name }}</td>
                                    <td>{{ $tt->program_number }}</td>
                                    <td>{{ $tt->crop_name }}</td>
                                    <td class="program-name">{{ $tt->date }}</td>
                                    <td>{{ $tt->venue }}</td>
                                    <td>{{ $tt->training_program_cost }}</td>
                                    <td>{{ $tt->as_center }}</td>
                                    <td class="button-container">
                                    @if(auth()->user()->hasPermission('tank_training_participants', 'add'))
                                    <a href="{{ route('tank-training-participants.create', $tt->id) }}" title="Add Participant">
                                        <button class="btn btn-success" style="height: 40px; width: 150px; font-size: 16px;">Add Participants</button>
                                    </a>
                                    @endif
                                    @if(auth()->user()->hasPermission('tank_training_participants', 'view'))
                                    <a href="{{ route('tank-training-participants.list', $tt->id) }}" title="View Participants">
                                        <button class="btn btn-success" style="height: 40px; width: 120px; font-size: 16px;">View Details</button>
                                    </a>
                                    @endif
                                    @if(auth()->user()->hasPermission('tank_training', 'edit'))
                                        <a href="{{ route('tank-training.edit', $tt->id) }}" class="btn btn-danger edit-button" title="Edit">
                                            <img src="{{ asset('assets/images/edit2.png') }}" alt="Edit Icon" style="width: 16px; height: 16px;">
                                        </a>
                                    @endif
                                    @if(auth()->user()->hasPermission('tank_training', 'delete'))
                                        <form action="{{ route('tank-training.destroy', $tt->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                        ...
                        <nav aria-label="Page navigation example">
                            <ul class="pagination">
                                <li class="page-item {{ $tankTrainings->onFirstPage() ? 'disabled' : '' }}">
                                    <a class="page-link" href="{{ $tankTrainings->previousPageUrl() }}">Previous</a>
                                </li>
                                @php $currentPage = $tankTrainings->currentPage(); $lastPage = $tankTrainings->lastPage(); $startPage = max($currentPage - 2, 1); $endPage = min($currentPage + 2, $lastPage); @endphp
                                @if ($startPage > 1)
                                    <li class="page-item"><a class="page-link" href="{{ $tankTrainings->url(1) }}">1</a></li>
                                    @if ($startPage > 2)<li class="page-item disabled"><span class="page-link">...</span></li>@endif
                                @endif
                                @for ($i = $startPage; $i <= $endPage; $i++)
                                    <li class="page-item {{ $i == $currentPage ? 'active' : '' }}">
                                        <a class="page-link" href="{{ $tankTrainings->url($i) }}">{{ $i }}</a>
                                    </li>
                                @endfor
                                @if ($endPage < $lastPage)
                                    @if ($endPage < $lastPage - 1)<li class="page-item disabled"><span class="page-link">...</span></li>@endif
                                    <li class="page-item"><a class="page-link" href="{{ $tankTrainings->url($lastPage) }}">{{ $lastPage }}</a></li>
                                @endif
                                <li class="page-item {{ $tankTrainings->hasMorePages() ? '' : 'disabled' }}">
                                    <a class="page-link" href="{{ $tankTrainings->nextPageUrl() }}">Next</a>
                                </li>
                            </ul>
                        </nav>

                        @php
                            $currentPage = $tankTrainings->currentPage();
                            $perPage = $tankTrainings->perPage();
                            $total = $tankTrainings->total();
    ...
    <script>
        $(document).ready(function() {
            $('#entriesSelect').change(function() {
                ...
                window.location = '{{ route('tank-training.index') }}?entries=' + perPage + (window.location.search.indexOf('search=') !== -1 ? '&' + window.location.search.slice(1) : '');
            });
        });
    </script>
    <script>
