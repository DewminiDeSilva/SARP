...
<a href="{{ route('training.index') }}" class="btn-back">
            ...
            <div class="container-fluid">
                <div class="center-heading text-center">
                    <h1 style="font-size: 2.5rem; color: green;">Training Program Details</h1>
                </div>
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <!-- CSV Upload Form -->
                    @if(auth()->user()->hasPermission('training', 'upload_csv'))
                    <form action="{{ route('training.upload_csv') }}" method="POST" enctype="multipart/form-data" class="form-inline">
                        @csrf
                    ...
                    @endif
                    <!-- Search form -->
                    <form method="GET" action="{{ route('searchTraining') }}" class="form-inline">
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <div class="entries-container">
                        ...
                        <select id="entriesSelect" class="custom-select custom-select-sm form-control form-control-sm mx-2">
                            <option value="10" {{ $trainings->perPage() == 10 ? 'selected' : '' }}>10</option>
                            <option value="25" {{ $trainings->perPage() == 25 ? 'selected' : '' }}>25</option>
                            <option value="50" {{ $trainings->perPage() == 50 ? 'selected' : '' }}>50</option>
                            <option value="100" {{ $trainings->perPage() == 100 ? 'selected' : '' }}>100</option>
                        </select>
                        ...
                        <table id="trainingTable" class="table table-bordered">
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    <th>Venue</th> <!-- Changed from Location to Venue -->
                                    <th>Resource Person</th> <!-- Changed from Program Conductor to Resource Person -->
                                    <th>Program Cost</th> <!-- Changed from Cost to Program Cost -->
                                    <th>Resource Person Payment</th> <!-- New field added -->
                                    <!-- <th>Province</th> -->
                                    <th>District</th>
                                    <th>DSD</th>
                                    <th>GND</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="program-name">{{ $training->date }}</td>
                                    <td>{{ $training->venue }}</td> <!-- Changed from 'place' to 'venue' -->
                                    <td>{{ $training->resource_person_name }}</td> <!-- Changed from 'conductor_name' -->
                                    <td>{{ $training->training_program_cost }}</td> <!-- Changed from 'conductor_payment' -->
                                    <td>{{ $training->resource_person_payment }}</td> <!-- New field for resource person payment -->
                                    <!-- <td>{{ $training->province_name }}</td> -->
                                    ...
                                    @if(auth()->user()->hasPermission('training', 'add'))
                                        <a href="{{ route('participants.create', $training->id) }}" title="Add Participant">
                                            <button class="btn btn-success" style="height: 40px; width: 150px; font-size: 16px;">Add Participants</button>
                                        </a>
                                    @endif
                                        <a href="{{ route('participants.list', $training->id) }}" title="View Participants">
                                            <button class="btn btn-success" style="height: 40px; width: 120px; font-size: 16px;">View Details</button>
                                        </a>
                                        @if(auth()->user()->hasPermission('training', 'edit'))
                                        <a href="/training/{{ $training->id }}/edit" class="btn btn-danger edit-button" title="Edit">
                                            <img src="{{ asset('assets/images/edit2.png') }}" alt="Edit Icon" style="width: 16px; height: 16px;">
                                        </a>
                                        @endif
                                        @if(auth()->user()->hasPermission('training', 'delete'))
                        ...
                        <nav aria-label="Page navigation example">
                            <ul class="pagination">
                                <li class="page-item {{ $trainings->onFirstPage() ? 'disabled' : '' }}">
                                    <a class="page-link" href="{{ $trainings->previousPageUrl() }}" tabindex="-1" aria-disabled="true">Previous</a>
                                </li>
                                @php
                                ...
                                @if ($startPage > 1)
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $trainings->url(1) }}">1</a>
                                    </li>
                                    @if ($startPage > 2)
                                ...
                                @if ($endPage < $lastPage)
                                    ...
                                    @endif
                                    <li class="page-item">
                                        <a class="page-link" href="{{ $trainings->url($lastPage) }}">{{ $lastPage }}</a>
                                    </li>
                                @endif
                        ...
                        <script>
                            $(document).ready(function() {
                                $('#entriesSelect').change(function() {
                                    ...
                                    window.location = '{{ route('training.index') }}?entries=' + perPage;
                                });
                            });
                        </script>
