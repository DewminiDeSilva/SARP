...
<div class="wrap">
  <div class="h1" style="margin-bottom:12px;">Project Goal Logframe — Create / Edit</div>

  <form method="POST" action="{{ route('logframe.project-goal.store') }}" class="grid" style="gap:16px;">
    @csrf
    ...
    <div class="card" style="padding:16px;">
      <div class="grid meta">
        ...
        <div class="field">
          ...
          <input type="number" min="0" name="mid_term" value="{{ old('mid_term', $meta['mid_term'] ?? 10500) }}">
        </div>
        <div class="field">
          <label>End Target</label>
          <input type="number" min="0" name="end_target" value="{{ old('end_target', $meta['end_target'] ?? 28000) }}">
        </div>

        <div class="field">
          <label>Source</label>
          <input type="text" name="source" value="{{ old('source', $meta['source'] ?? 'Reference studies and HH surveys') }}">
        </div>
        <div class="field">
          <label>Frequency</label>
          <input type="text" name="frequency" value="{{ old('frequency', $meta['frequency'] ?? 'Baseline, MTR, End-line') }}">
        </div>
        <div class="field">
        ...
        <div class="field" style="grid-column:1 / -1;">
          ...
          <textarea rows="2" name="assumptions">{{ old('assumptions', $meta['assumptions'] ?? 'Extreme climate change shocks do not occur') }}</textarea>
        </div>
      </div>
    </div>
    ...
    <div class="btns" style="display:flex;justify-content:center;gap:8px;">
      ...
      <a href="{{ route('logframe.project-goal.index') }}" class="btn">Cancel</a>
    </div>
  </form>
</div>
@endsection
